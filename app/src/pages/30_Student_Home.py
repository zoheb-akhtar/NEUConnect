...
st.title(f"Welcome Student, {st.session_state.get('first_name', 'Timmy')}!")
st.write('')
st.write('')
...
if st.button('🔍 Browse Alumni Mentors', 
    ...
    st.switch_page('pages/Browse_Alumni.py')

if st.button('👥 My Connections', 
             type='primary',
             use_container_width=True):
    st.switch_page('pages/My_Connections.py')

if st.button('📅 My Sessions', 
             type='primary',
             use_container_width=True):
    st.switch_page('pages/My_Sessions.py')

if st.button('📝 My Notes & Advice', 
             type='primary',
             use_container_width=True):
    st.switch_page('pages/My_Notes.py')
